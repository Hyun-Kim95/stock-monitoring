...
3. 개발 서버
  ...
  - 설정 UI: [http://localhost:3000/settings/stocks](http://localhost:3000/settings/stocks)
...
`ADMIN_API_TOKEN`을 설정한 경우, 루트 `.env`의 `NEXT_PUBLIC_ADMIN_TOKEN`을 같은 값으로 맞춘 뒤 웹을 다시 띄웁니다. 둘 다 비우면 로컬에서 설정·관리 API용 Bearer 인증이 생략됩니다.
