...
PRD 기준 MVP 완료를 위한 작업 분해 목록입니다. 순서는 **의존 관계(데이터 소스 확정 → 백엔드/DB → 실시간 → 뉴스 → 사용자 UI → 설정 UI → 배포)**를 따릅니다.
...
| 1 | **완료** (npm workspaces; 루트 `package.json`의 `workspaces`) |
| 2~3, 6~7 | **완료** (MVP) |
...
| 9 | `DEPLOYMENT.md`, GitHub Actions CI **추가**; 전 항목 완료는 아님 |
| 10 | 프로덕 E2E 동시 확인 **미완** |
...
- [x] 설정 UI·API 경로 보호 방식 결정 (예: 환경변수 시크릿 토큰, Basic Auth, IP 제한, 리버스 프록시 규칙) — MVP: `ADMIN_API_TOKEN` Bearer; 운영은 `DEPLOYMENT.md` 참고
- [x] API 키 저장 방침 (서버 전용, 암호화 여부, 설정 UI 마스킹 규칙) — 설정 API 마스킹·서버 전용 env
...
> | 패키지 매니저 | **npm** 10+ (npm workspaces) | 루트 `package.json`의 `workspaces` |
> | 레포 형태 | **모노레포** `apps/` + `packages/` | API와 WS를 Next와 분리 |
> | 프론트 | **Next.js 15** App Router + **TypeScript** | PC 대시보드 + 설정 UI 라우트 그룹 `(admin)` |
> | API·실시간 | **Fastify** + **@fastify/websocket** + TypeScript | REST + 브라우저용 WS 한 프로세스 |
> | DB | **PostgreSQL 16+** | 로컬 설치 또는 호스팅 |
...
```text
stockMonitoring/
├── package.json                 # private: true, workspaces: apps/* packages/*, 공통 스크립트
├── .nvmrc                       # 22
...
│   │   │   │   ├── page.tsx                    # 메인 대시보드
│   │   │   │   └── (admin)/
│   │   │   │       ├── layout.tsx              # 설정 UI 공통 레이아웃
│   │   │   │       ├── settings/stocks/page.tsx
│   │   │   │       ├── settings/themes/page.tsx
...
- [x] `private: true`, **npm workspaces** (`workspaces`: `apps/*`, `packages/*`; `packageManager` 필드 생략 가능)
- [x] `workspaces`: `apps/*`, `packages/*`
- [x] 스크립트(실제 루트 `package.json` 기준)  
  - `dev`: `npm run dev` (api + web 동시)  
  - `db:generate` / `db:migrate` / `db:seed`: `npm run db:*` (루트 스크립트가 Prisma를 호출)  
  - `lint` / `typecheck` / `test`: 전 패키지  
  - (참고) pnpm을 쓸 경우 `pnpm -r`·`pnpm --filter @stock-monitoring/db exec …` 등으로 대응 가능하나, **본 레포 표준은 npm**이다.
...
- [x] 절대 경로 별칭 `@/` → `src/`
- [x] 대시보드는 `src/app/page.tsx`, 설정 UI는 `src/app/(admin)/settings/...` 로 URL 네임스페이스 분리 (`/settings/*`)
...
- [x] (선택) `@fastify/rate-limit` — 보호된 쓰기 라우트에만 — **대체:** 비-GET 요청 IP당 분당 120회 `write-rate-limit.ts`
...
| `ADMIN_API_TOKEN` | api, web | 설정·관리 REST용 `Authorization: Bearer` |
...
- [x] 배포: 호스트 프로세스 + `DEPLOYMENT.md` 체크리스트
...
- [x] PostgreSQL 로컬: OS 패키지/공식 설치본 등 **16+** 권장
...
- [x] 초기 마이그레이션: `npm run db:migrate` (Prisma `migrate dev`; 이름은 프롬프트 또는 `--name`으로 지정)
...
### 3.5 공통
- [x] 입력 검증, 404/409 처리
- [x] 보호된 쓰기 라우트에 Phase 0.3에서 정한 보호 적용
...
### 4.2 구독 집합 관리
- [x] DB 활성 종목 코드 집합 로드
- [x] `OWNER`/`ADMIN`이 종목 변경 시 **구독 목록 갱신** 경로 (재시작 없이 또는 재시작 허용 명시)
...
- [ ] 로그 수집 위치·로테이션 최소 설정
- [ ] “외부 API 장애” 시 사용자 화면 문구·운영자 확인 방법 정리

---

## Phase 10 — MVP 완료 체크 (PRD §5.2)

- [x] 관심종목 등록/조회 동작 (설정 UI 등록 + 대시보드 조회)
- [x] 실시간 시세 수신 및 화면 반영 — **목** 프로바이더
- [x] 종목별 뉴스 리스트 조회 — **목** 뉴스
- [x] 설정 UI: 종목, 테마, 뉴스 소스, 시스템 설정 관리
...
- **A트랙**: Phase 2~3 REST + Phase 7 설정 UI
...
*문서 버전: PRD MVP 기준. Phase 1~2는 **Next.js 15 + Fastify + PostgreSQL + Prisma + npm workspaces 모노레포** 기준으로 상세화함. 스택을 바꿀 경우 Phase 1 트리·패키지명·스크립트를 그에 맞게 치환하면 됨. 시세/뉴스 API 확정 후 Phase 4·5 세부 TODO를 해당 벤더 스펙에 맞게 조정하세요.*
