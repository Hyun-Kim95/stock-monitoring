...
tags: [settings-ui, operations]
related_prd: ../settings-ui-prd.md
---

# 설정 UI·운영 — 배포 메모

PRD [`settings-ui-prd.md`](../settings-ui-prd.md) §3·§6·§9 및 루트 `README.md`와 정렬한다.

## 환경 변수 (설정 UI·보안)

| 변수 | 용도 |
|------|------|
| `ADMIN_API_TOKEN` | 설정 시 보호된 쓰기(adminPre)에 `Authorization: Bearer …` 허용; 미설정 시 **세션+OWNER/ADMIN만** 유효 |
...
**주의:** 루트 `README`의 `NEXT_PUBLIC_ADMIN_TOKEN`은 대시보드 등 안내용일 수 있으나, 공용 웹 `api-client`는 **`Authorization`을 자동 부착하지 않는다.** 브라우저 설정 화면(`/settings/*`)은 **로그인 세션 + `OWNER`/`ADMIN`** 이 기본이다. 운영 문서에서 **`ADMIN_API_TOKEN`(서버)** 과 **`NEXT_PUBLIC_ADMIN_TOKEN`(클라이언트 빌드 노출)** 을 혼동하지 않도록 구분한다.

## 배포 전 체크리스트

1. **무단 변경 방지:** 스테이징/프로덕션에서 `ADMIN_API_TOKEN` 설정 **또는** 세션 기반(`OWNER`/`ADMIN`)만으로 충분한지 확인.
...
| 증상 | 우선 확인 |
|------|-----------|
| 설정 UI 저장 시 **403** `CSRF_REJECTED` | 브라우저 출처가 허용 목록에 있는지, 리버스 프록시가 `Origin`/`Referer`를 깨지 않는지 |
...
| **500**·비정상 본문 | Bearer만 통과·세션 없는 보호된 쓰기 등 예외 경로 — PRD §0·§7.4·서버 로그 확인 |
