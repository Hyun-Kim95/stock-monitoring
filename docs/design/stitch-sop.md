...
MCP(`user-stitch`) 없이 **터미널 CLI**로만 작업하는 경우, 위 표준 순서는 **개념적 SOP**로만 쓰고 실제 명령은 설치한 CLI의 도움말을 따른다.  
설정 UI 예시·프롬프트·산출물 기록 형식은 [`settings-ui-stitch-option-b-cli.md`](./settings-ui-stitch-option-b-cli.md)를 참고한다.
