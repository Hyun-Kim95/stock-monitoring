...
- **운영자(`OWNER`/`ADMIN`):** `/settings/stocks`에서 수동으로 **저장**하면 `PATCH /stocks/:id`로 반영된다. 종목코드는 수정 폼에서 바꿀 수 없다.
- **뉴스 검색:** 공식명이 바뀌면 API가 **이전 공식명을 `searchAlias`에 자동 병합**(중복 제외)해, 제목에 구명만 남은 기사도 네이버 검색어에 잡히게 한다.
- **뉴스 규칙:** `NewsSourceRule`의 include/exclude 키워드가 구명에 묶여 있으면 이름 변경 후 **수동으로 규칙을 점검**한다(`/settings/news-rules`).
