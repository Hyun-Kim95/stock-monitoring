#!/usr/bin/env node
/**
 * 안 B: @google/stitch-sdk 로 관리자 데스크톱 화면 1건 생성 + 결과 URL 기록.
 *
...
const PROMPT = [
  "Korean stock dashboard admin web, desktop 1280px.",
  "Left sidebar: title Settings, email line, back link, nav items (Stock admin, Theme admin, News rules, Runtime settings), logout.",
  "Main: header Stock admin, pill badge Active 12 / Max 100.",
...
async function main() {
  if (!process.env.STITCH_API_KEY?.trim()) {
    console.error("[stitch-generate-admin] STITCH_API_KEY 가 필요합니다. 루트 .env 에 설정 후 실행하세요.");
  ...
  if (!projectId) {
    console.error(
      "[stitch-generate-admin] 사용 가능한 Stitch 프로젝트가 없습니다. Stitch 웹/UI에서 프로젝트를 만든 뒤 STITCH_PROJECT_ID 를 지정하거나, API로 생성 가능한지 계정을 확인하세요.",
    );
    process.exitCode = 1;
    return;
  }
  ...
  const summary = `# Stitch 생성 결과 (관리자 화면 · 안 B)
...
확정 시 docs/design 의 admin-site-design-comparison.md 에 반영하세요.
`;

  await writeFile(join(outDir, "run-summary.md"), summary, "utf8");
  console.log("[stitch-generate-admin] 기록됨:", join(outDir, "run-summary.md"));
}

main().catch((e) => {
  console.error("[stitch-generate-admin]", e);
  process.exitCode = 1;
});
